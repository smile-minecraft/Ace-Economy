...
**修正：** 使用 Java 25 配 Paper/Folia 26.1.2，安裝一個 `AceLib-1.2.0.jar`，然後完整重啟伺服器。缺少硬依賴時，`/aceeco reload` 救不了你。
...
**可能原因：** YAML 無效、v2 的鍵結構錯誤、值不合法、語言檔載入不了，也可能是 reload 刻意拒絕的貨幣或銀行介面變更。
...
**修正：** 還原到最後一份確認正常的版本，再執行 `/aceeco reload`。重新載入失敗時會保留最後一份有效的記憶體設定，不要以為那份改到一半的檔案已經生效。如果拒絕理由是結構性貨幣變更，先把檔案還原，讓 reload 恢復成功；真正的變更排進維護重啟：先備份、停服、確認檔案、啟動，再看啟動日誌有無設定錯誤。純顯示變更不用重啟，下一次 reload 就會成功。只有檔案有效之後，或者改動的是啟動期的儲存／依賴設定時，才需要完整重啟。
...
**修正：** 改動了 JAR、AceLib、`storage.type`、資料庫連線值或選用插件的可用狀態之後，請完整重啟。reload 回報點名的 `settings.main-command-alias`、`storage.type`、`leaderboard.enabled` 與結構性貨幣變更也一樣：都要完整重啟才會生效。成功的 reload 會關掉已開啟的銀行介面，屬於正常現象；請玩家重開 `/bank` 就好。不要用 Bukkit 的 `/reload` 來做正式升級或復原。
