...
reload 是一整筆交易：設定、語言、貨幣、銀行介面四組候選會先各自驗證，全部通過才一起換上。任一組失敗就什麼都不換，伺服器沿用舊設定繼續跑。
...
有三個設定只能重啟，不會被 reload 套用。`settings.main-command-alias`、`storage.type`、`leaderboard.enabled` 如果和運作中的值不同，reload 照樣成功，但回報會多一條點名的重啟提醒；線上值維持不動，下次完整重啟才會換。

回報會告訴你發生了什麼事。失敗時顯示拒絕理由，例如哪個貨幣 ID 被新增、哪個小數位數被改，讓你知道該修檔案還是排重啟。成功時附加備註：哪些已熱套用（顯示文字、版面、設定與語言），哪些還要重啟。

成功的 reload 也會在換上新版面前關掉所有已開啟的銀行介面，不讓任何點擊卡在新舊規則之間；受影響的玩家重開 `/bank` 就好。跟 reload 撞期的開啟會被拒絕，並用新的版面只重試一次；重試又撞期就直接失敗，玩家重開 `/bank` 即可。
