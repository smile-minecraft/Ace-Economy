...
| 持久化 `1` | SQL 資料表 `ace_v2_schema`，或 JSON `schemaVersion` | 儲存方式目前理解的 v2 持久化模型。 |
...
這張表記錄持久化模型版本。欄位包含 `version` 與 `updated_at`；初始化的 v2 儲存位置會寫入版本 `1`。這一列用來告訴儲存方式正在開啟哪個模型，不包含玩家餘額。
...
| `timestamp` | 事件時間。 |
| `reason` | 事件可附帶的原因。 |
| `reverted` | 這筆紀錄是否已被標記為已撤銷（`reverted`）。 |

兩種 SQL 方言都以十進位文字儲存金額與餘額快照。MySQL 用 v2 的 `VARCHAR` 存金額，並用布林值記錄 `reverted` 標記；SQLite 用文字存金額、用整數存布林值。兩邊的邏輯欄位相同。
...
這個範例刻意保持精簡。實際快照可以包含許多帳戶與交易。`accounts` 保存餘額，`transactions` 保存事件歷史與 `reverted` 狀態。

## 新的 SQL 儲存位置如何初始化

SQL 後端在沒有 v2 資料表的全新位置啟動時，會建立：
...
既有的相容儲存位置可以重複執行建表，不會重複寫入資料。應由插件初始化資料表；管理員不需要把舊版 v1 DDL 腳本複製到新的 v2 資料庫。
...
4. 如果已有 v2 JSON 快照，將該快照還原到新的後端。
5. 正式運作前，確認帳戶餘額與設定中的貨幣 ID。

這套模型沒有描述舊版 v1 列與 v2 帳戶、餘額、交易之間的自動轉換。v2 快照帶有共用的邏輯模型，因此可在 JSON 與 SQL 後端間搬移；這不代表舊版 v1 備份也相容。

## 備份與復原

操作順序請參考[持久化、備份與還原](persistence.zh-TW.md)。需要保留的完整資料邊界是 v2 模型：帳戶、餘額、交易，以及 schema 版本。

替換正式儲存位置前，請先停止伺服器並保留目前的備份。還原時會先解析輸入的 v2 JSON 快照，再檢查其 schema 版本。解析或版本檢查失敗時，現有資料會保留。成功還原後，後端會在還原操作中用快照替換目前的帳戶、餘額與交易。
...
如果既有資料庫只有那些舊資料表名稱，請不要直接讓 v2 後端指向它並假設會接收舊資料。請保留舊備份，建立或選擇 v2 儲存位置，並把資料轉換當成另外的遷移決策。
