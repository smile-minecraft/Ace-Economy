...
AceEconomy 執行時需要 AceLib `v1.2.1`。請先裝好 AceLib，再啟動 AceEconomy。`plugin.yml` 把 AceLib 列為硬依賴；如果沒有一個就緒的 AceLib 服務，AceEconomy 根本不會啟動。執行環境還需要 Java 25，以及符合本版本基準的 Paper/Folia 伺服器。
...
名稱型 Vault 方法（`hasAccount(String)`、`getBalance(String)`、`has(String, ...)`、`depositPlayer(String, ...)`、`withdrawPlayer(String, ...)`、`createPlayerAccount(String)`，包含世界名稱多載）只用線上玩家和已經快取的離線紀錄來認名字，呼叫的當下不會去讀儲存或打網路。比對名字時不分大小寫，帳戶認的始終是 UUID，所以改名後的玩家還是同一個帳戶。未知或空白的名稱永遠不會被當成「餘額為 0 的有效帳戶」：查餘額或 `has` 只會回 `0.0`／`false`，並在伺服器日誌留一筆 `FINE` 等級的診斷；存款或提款則回 `FAILURE` 並說明原因。世界名稱參數收下但不會用：沒有分世界的餘額，所有查詢回的都是全域餘額。
...
- 請求被拒絕、超時、映射失敗或傳輸失敗時，通知器會忽略這個錯誤；已經提交的經濟結果保持不變。
- 內容欄位有長度限制，設定裡的密鑰會被遮蔽；webhook 網址本身不會放進內容裡。
...
- **內容裡出現了密鑰：** 從交易文字裡移除那個值，並輪換密鑰。webhook 憑證只能放在本機設定裡。
...
`/bank open` 會給基岩版玩家原生表單，而不是箱子選單：首頁顯示餘額與存款、提領、關閉三個按鈕；提領要輸入金額與幣別，再經過確認步驟才會執行。兩種介面走同一條存提款路徑。關閉、無效、離線或重新載入後的回應一律不會執行交易；真機驗收仍在等待——自動化測試已覆蓋路由與安全規則，但在真實客戶端完成存款、提領、取消與重開之前，不要宣稱表單流程可用。

### 限制

- 只有點擊動作會降級；懸停文字在基岩版不保證顯示。
- Floodgate 沒裝、被停用或查詢失敗時，所有玩家都收到原始訊息並維持箱子銀行選單——啟動流程與 Java 行為完全不變。
- 提示文字來自 `lang/<locale>.yml` 的 `message.bedrock.fallback.*` 鍵；內容以純文字插入，不會被當成格式解析。銀行表單文字來自同一檔案的 `gui.bank-form-*` 鍵。

## 相關指南

- [整合 API](integration-api.zh-TW.md) — 給插件開發者的 Vault 與 PlaceholderAPI 公開合約。
