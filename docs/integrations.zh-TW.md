...
AceEconomy 執行時需要 AceLib `v1.0.0`。請先裝好 AceLib，再啟動 AceEconomy。`plugin.yml` 把 AceLib 列為硬依賴；如果沒有一個就緒的 AceLib 服務，AceEconomy 根本不會啟動。執行環境還需要 Java 25，以及符合本版本基準的 Paper/Folia 伺服器。
