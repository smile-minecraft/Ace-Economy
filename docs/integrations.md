...
AceEconomy requires AceLib `v1.2.1` at runtime. Install it before AceEconomy; `plugin.yml` declares AceLib as a hard dependency, so AceEconomy will not start without a ready AceLib service. It also requires Java 25 and a Paper/Folia server matching the release baseline.
...
2. Check that the AceLib version is `v1.2.1` for this release.
