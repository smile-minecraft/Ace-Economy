...
reload 是一整笔交易：配置、语言、货币、银行界面四组候选会先各自验证，全部通过才一起换上。任一组失败就什么都不换，服务器沿用旧配置继续跑。
...
有三个设置只能重启，不会被 reload 套用。`settings.main-command-alias`、`storage.type`、`leaderboard.enabled` 如果和运行中的值不同，reload 照样成功，但回报会多一条点名的重启提醒；线上值保持不动，下次完整重启才会换。

回报会告诉你发生了什么事。失败时显示拒绝理由，例如哪个货币 ID 被新增、哪个小数位数被改，让你知道该修文件还是排重启。成功时附加备注：哪些已热套用（显示文字、布局、配置与语言），哪些还要重启。

成功的 reload 也会在换上新布局前关闭所有已打开的银行界面，不让任何点击卡在新旧规则之间；受影响的玩家重开 `/bank` 就好。跟 reload 撞期的打开会被拒绝，并用新的布局只重试一次；重试又撞期就直接失败，玩家重开 `/bank` 即可。
