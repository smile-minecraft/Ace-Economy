...
The v2 runtime requires Java 25, Paper/Folia 26.1.2, and `AceLib-1.2.1.jar`. The v2 plugin is `AceEconomy-2.1.0.jar`. Vault and PlaceholderAPI remain optional integrations. Paper/Folia 26.1.2 is the officially supported server line; Folia 26.2 has been validated only on specific builds (VERIFIED-BETA), and other 26.2 builds are unverified.
...
3. Move `AceEconomy-2.1.0.jar` and `AceLib-1.2.1.jar` out of the live `plugins/` directory.
