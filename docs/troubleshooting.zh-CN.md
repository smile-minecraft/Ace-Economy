...
**修正：** 修正 YAML 里的嵌套结构，重新启动服务器，再看第一条存储相关的消息。
...
**修正：** 把这两个键改对，然后执行 `/aceeco reload`；如果插件或整合组合变了，再重启。Discord 的通知是异步、尽力而为的，通知没到不代表钱没动。通知失败不会撤销已经完成的交易，所以要分别去确认玩家余额和交易结果。
...
**可能原因：** Floodgate 没装或被禁用、该玩家是 Java 客户端，或者查询失败、消息退回原始版本。

**先检查：** 确认 Floodgate 已经安装并启用，且受影响的玩家确实是用基岩版加入的。Java 玩家本来就会保留可点击按钮，永远看不到提示。

**修正：** 安装或启用 Floodgate，然后重启。不要宣称完整基岩版支持：hover 文字不保证显示，只有点击动作会降级。Floodgate 不在时看到原始消息是预期行为，不是错误。

## 配置重新加载失败

**可能原因：** YAML 无效、v2 的键结构错误、值不合法、语言文件加载不了，也可能是重新加载时会拒绝的货币或银行界面变更。

**先检查：** 查看 `plugins/AceEconomy/config.yml` 最近一次改了什么，以及 `plugins/AceEconomy/lang/` 里选中的文件。确认配置仍然含有 `version: "2.0"`，并且用到的 `storage.sqlite` 和 `storage.mysql` 都是映射。如果回报点名了某个货币 ID，先看清楚改了什么：显示文字（`name`、`symbol`）还是结构（新增／删除 ID、`scale`、`default`）。

**修正：** 还原到最后一份确认正常的版本，再执行 `/aceeco reload`。重新加载失败时会保留最后一份有效的内存配置，不要以为那份改到一半的文件已经生效。如果拒绝理由是结构性货币变更，先把文件还原，让重新加载恢复成功；真正的变更排进维护重启：先备份、停服、确认文件、启动，再看启动日志有无配置错误。纯显示变更不用重启，下一次重新加载就会成功。只有文件有效之后，或者改动的是启动期的存储／依赖设置时，才需要完整重启。

## 重新加载、重启或停服行为异常

先分清楚这三种操作：

- `/aceeco reload` 重新加载配置和语言文件。失败的回报会给拒绝理由，成功的回报会附加已套用与待重启备注。
- 完整重启会重新打开存储后端，并重新加载插件依赖。
- `stop` 执行正常的停服；请等保存完成。

**修正：** 改动了 JAR、AceLib、`storage.type`、数据库连接值或可选插件的可用状态之后，请完整重启。重新加载回报点名的 `settings.main-command-alias`、`storage.type`、`leaderboard.enabled` 与结构性货币变更也一样：都要完整重启才会生效。重新加载成功会关闭已打开的银行界面，属于正常现象；请玩家重开 `/bank` 就好。不要用 Bukkit 的 `/reload` 来做正式升级或恢复。
...
**修正：** 在确认后端和货币之前，先停掉其他余额变动。只有在停服时，才能从确认正常的 v2 备份还原。不要在正式存储上反复尝试没把握的修复。
