...
**修正：** 使用 Java 25 配 Paper/Folia 26.1.2，安装一个 `AceLib-1.2.0.jar`，然后完整重启服务器。缺少硬依赖时，`/aceeco reload` 救不了你。
...
**可能原因：** YAML 无效、v2 的键结构错误、值不合法、语言文件加载不了，也可能是 reload 刻意拒绝的货币或银行界面变更。
...
**修正：** 还原到最后一份确认正常的版本，再执行 `/aceeco reload`。重新加载失败时会保留最后一份有效的内存配置，不要以为那份改到一半的文件已经生效。如果拒绝理由是结构性货币变更，先把文件还原，让 reload 恢复成功；真正的变更排进维护重启：先备份、停服、确认文件、启动，再看启动日志有无配置错误。纯显示变更不用重启，下一次 reload 就会成功。只有文件有效之后，或者改动的是启动期的存储／依赖设置时，才需要完整重启。
...
**修正：** 改动了 JAR、AceLib、`storage.type`、数据库连接值或可选插件的可用状态之后，请完整重启。reload 回报点名的 `settings.main-command-alias`、`storage.type`、`leaderboard.enabled` 与结构性货币变更也一样：都要完整重启才会生效。成功的 reload 会关闭已打开的银行界面，属于正常现象；请玩家重开 `/bank` 就好。不要用 Bukkit 的 `/reload` 来做正式升级或恢复。
