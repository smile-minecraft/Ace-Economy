...
AceEconomy 執行時需要 AceLib `v1.2.1`。Vault 與 PlaceholderAPI 是選用整合；你的插件應該能處理其中任一公開服務不可用的情況。如果標準整合已經夠用，不要把 AceEconomy 的實作類別設成硬依賴。
...
- 不要自動重試失敗的 Vault 交易，除非你自己的產品合約明確定義了安全的重試策略。

## 公開整合清單

插件整合應該依賴公開合約，而不是實作細節：
