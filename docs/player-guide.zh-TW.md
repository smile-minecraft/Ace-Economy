...
用 `/bank open` 開啟銀行面板，手持銀行支票並點擊存款按鈕（上方中間那一格），金額就會存入你的帳戶，支票也會從手中消失；直接手持支票按右鍵也能兌回，兩種方式走同一條原子入帳路徑。每張支票只能兌回一次，伺服器會永久記住已兌換過的支票。如果存款被拒絕（物品無效、已經使用過或貨幣不明），支票會留在你手上；請保留物品並找管理員處理，不要嘗試複製交易。萬一錢已經入帳、支票卻拿不掉，兩邊都會留著：先把支票收好並聯絡管理員，管理員查過交易紀錄後會幫你人工補上。
