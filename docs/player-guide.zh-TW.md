...
用 `/bank open` 開啟銀行面板，手持銀行支票並點擊存款按鈕（上方中間那一格），金額就會存入你的帳戶，支票也會從手中消失；直接手持支票按右鍵也能兌回，兩種方式走同一條原子入帳路徑。每張支票只能兌回一次，伺服器會永久記住已兌換過的支票。如果存款被拒絕（物品無效、已經使用過或貨幣不明），支票會留在你手上；請保留物品並找管理員處理，不要嘗試複製交易。
