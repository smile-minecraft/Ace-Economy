...
对于已有的兼容存储，建表可以重复执行，不会重复写入数据。应由插件初始化这些表；管理员不需要把旧版 v1 DDL 脚本复制到新的 v2 数据库。
...
这套模型没有描述旧版 v1 行与 v2 账户、余额、交易之间的自动转换。v2 快照带有共用的逻辑模型，因此可以在 JSON 与 SQL 后端之间转移；这不代表旧版 v1 备份也兼容。
...
如果现有数据库只有这些旧表名，不要直接让 v2 后端指向它，并假设旧行会被接收。保留旧备份，创建或选择 v2 位置，并把数据转换作为单独的迁移决策处理。
