...
AceEconomy 运行时需要 AceLib `v1.2.1`。请先装好 AceLib，再启动 AceEconomy。`plugin.yml` 把 AceLib 声明为硬依赖；如果没有一个就绪的 AceLib 服务，AceEconomy 根本不会启动。运行环境还需要 Java 25，以及符合本版本基准的 Paper/Folia 服务器。

Vault、PlaceholderAPI 和 Floodgate 是可选的软依赖。插件没装或没启用时，AceEconomy 会跳过对应的整合。Discord 不需要另外装服务器插件，它用的是 `config.yml` 里配置的 webhook。

## AceLib

请安装与本版本匹配的 AceLib，再启动带 AceEconomy 的服务器。AceLib 必须先就绪，AceEconomy 才能注册指令、消息和整合；如果 AceLib 缺失或还没就绪，插件会直接停用，而不是带着残缺的服务勉强跑。

无法启动时：

1. 确认 AceLib 的 JAR 已经安装并启用。
2. 确认本版本使用的是 AceLib `v1.2.1`。
...
名称型 Vault 方法（`hasAccount(String)`、`getBalance(String)`、`has(String, ...)`、`depositPlayer(String, ...)`、`withdrawPlayer(String, ...)`、`createPlayerAccount(String)`，包含世界名称重载）只用在线玩家和已经缓存的离线记录来认名字，调用的当时不会去读存储或打网络。匹配名字时不区分大小写，账户认的始终是 UUID，所以改名后的玩家还是同一个账户。未知或空白的名称永远不会被当成“余额为 0 的有效账户”：查余额或 `has` 只会返回 `0.0`／`false`，并在服务器日志留一条 `FINE` 级别的诊断；存款或提款则返回 `FAILURE` 并说明原因。世界名称参数收下但不会用：没有分世界的余额，所有查询返回的都是全局余额。
...
- **交易报告失败：** 检查玩家账户和金额。失败的操作不会被报成成功，Vault 的转接器也不会重试。
...
- 内容的字段有长度限制，设置里的密钥会被遮蔽；webhook 网址本身不会放进内容里。
...
- **内容里出现了密钥：** 从交易文字里移除那个值，并轮换密钥。webhook 凭证只能放在本机配置里。
...
`/bank open` 会给基岩版玩家原生表单，而不是箱子菜单：首页显示余额与存款、提取、关闭三个按钮；提取要输入金额与货币，再经过确认步骤才会执行。两种界面走同一条存取款路径。关闭、无效、离线或重新加载后的回应一律不会执行交易；真机验收仍在等待——自动化测试已覆盖路由与安全规则，但在真实客户端完成存款、提取、取消与重开之前，不要宣称表单流程可用。

### 限制

- 只有点击动作会降级；悬停文字在基岩版不保证显示。
- Floodgate 没装、被停用或查询失败时，所有玩家都收到原始消息并维持箱子银行菜单——启动流程与 Java 行为完全不变。
- 提示文字来自 `lang/<locale>.yml` 的 `message.bedrock.fallback.*` 键；内容以纯文本插入，不会被当成格式解析。银行表单文字来自同一文件的 `gui.bank-form-*` 键。

## 相关指南

- [整合 API](integration-api.zh-CN.md) — 给插件开发者的 Vault 与 PlaceholderAPI 公开合约。
