...
本文說明 v2 `operations` 層（Task 11）的實際 API 與行為契約，讀者為 Task 12 wiring、release operator 與維護者。

**狀態邊界**：本文以目前原始碼與測試為準。CompositionRoot 尚未接入這些 service（Task 12 未完成），真實 Folia 伺服器驗證也尚未執行（Task 13 未完成）。文中所有簽章皆存在於 `src/main/java/com/smile/aceeconomy/operations/**` 與 `ports/operations/**`；未驗證事項列於最後一節。
...
- **executor 契約**：實作必須「全部生效或全部不生效」；`InMemoryReversalExecutor` 是循序執行（先 `appendBatch` 再存帳戶），對 in-memory repository 原子，但 production binding 應把餘額變更與 record append 包進單一 storage transaction 才能真正滿足契約。
...
`LeaderboardRow` 是標準化 row（`accountId`、`ownerName`、`balance`）。目前唯一實作是 `InMemoryLeaderboardSource`；SQL/JSON 背後的實作可在後續任務以同一 port 契約滿足，**本 slice 不包含、也未接線**。ranking、cache、分頁邏輯與後端無關。

---

## 4. ImportService — Essentials/CMI 匯入
...
- **vendor parsing 界線**：本 service 只接受已標準化的 `ImportRecord`；Essentials/CMI export 檔的解析與 vendor 檔案探索在**本 slice 之外**。本 slice 也不做任何自動 v1 → v2 資料 migration。
...
- **Rollback／匯入前先備份**。兩者都會改餘額、append audit records（匯入還會消費 idempotency key），實務上不可逆。備份／restore 程序見 `docs/persistence.md`（`backup()` / `restore()`，JSON 與 SQL 後端互通）。
- **`MARK_FAILED` 不要直接重試**（見 2.4）：先人工核對帳戶與 audit records。
- **本文件沒有 live 驗證**：MySQL 路徑只有 offline contract tests（見 `docs/persistence.md`），本環境沒有 live DB、vendor plugin 或 Folia runtime；所有行為來自原始碼與單元測試。
- **Task 12（CompositionRoot wiring）未完成**：上述 service 都以 ports 直接建構，尚未接入 plugin 生命週期；真實 binding（production `ReversalExecutor`、SQL/JSON `LeaderboardSource`、持久化 `IdempotencyGuard`）需在 Task 12 提供。
- **Task 13（真實伺服器驗證）未完成**：fresh-install 啟動、RCON／遊戲內驗證、故障演練與發布文件都尚未執行。