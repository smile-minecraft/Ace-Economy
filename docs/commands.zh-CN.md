...
存入一张有效的 v2 银行支票时，必须先完成持久防重播机制与入帐，之后才会移除支票或减少堆叠数量。无效、被重播或入帐失败的支票，会留在玩家物品栏里。手持支票按右键可直接兑换，走与银行面板存款按钮相同的原子入账路径。如果入账已经完成但支票拿不掉，这笔钱照样算数，整张支票也会留在手上；服务器审计日志会记下支票编号、玩家和入账金额，方便后面人工补发：先让玩家收好支票并联系管理员，管理员核对审计日志里的支票编号、收回或作废重复的支票后，再用比如 `/aceeco give` 把钱补上。
