...
存入一張有效的 v2 銀行支票時，必須先完成持久防重播機制與入帳，之後才會移除支票或減少堆疊數量。無效、被重播或入帳失敗的支票，會留在玩家物品欄裡。手持支票按右鍵可直接兌回，走與銀行面板存款按鈕相同的原子入帳路徑。如果入帳已經完成卻拿不掉支票，這筆錢仍然算數，整張支票也會留在手上；伺服器稽核紀錄會記下支票編號、玩家與入帳金額，方便之後人工補發：請玩家先收好支票並聯絡管理員，管理員對過稽核紀錄上的支票編號、收回或作廢重複的支票後，再用例如 `/aceeco give` 補上金額。
