...
基岩版玩家（Geyser + Floodgate）開啟的是原生表單：首頁顯示餘額與存款、提款、關閉三個按鈕；提領要輸入金額與幣別，再經過確認步驟才會執行。關閉表單、輸入無效的值、離線，或在回應前遇到 reload／斷線，都不會執行任何交易。兩種介面走同一條存提款路徑，nonce、防重播與 region 保證兩邊都成立。Floodgate 缺席時所有玩家都維持箱子選單。
