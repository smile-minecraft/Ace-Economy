...
AceEconomy 运行时需要 AceLib `v1.2.1`。Vault 和 PlaceholderAPI 是可选整合；你的插件应该能处理其中任一公开服务不可用的情况。如果标准整合已经够用，不要把 AceEconomy 的实现类设成硬依赖。
...
| 存款或提款失败 | `EconomyResponse` 报告失败，金额为 `0`，余额为当前值或零；不会重试。 |
...
| `symbol` | 格式化余额使用的前缀。 |
| `scale` | 货币定义的小数精度。 |
| `default` | 选择通过 Vault 和默认占位符形式公开的唯一货币。 |
...
- 不要自动重试失败的 Vault 交易，除非你自己的产品合约明确规定了安全的重试策略。

## 公开整合清单

插件整合应该依赖公开合约，而不是实现细节：
