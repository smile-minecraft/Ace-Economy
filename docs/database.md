...
Do not use the persistence value as the configuration value. The names below are the v2 names.
...
Creating the tables is safe to repeat for an existing compatible store. The plugin initializes the tables; administrators do not need to create them by hand.

## Upgrade path

The v2 configuration and persistence model are clean boundaries. Plan a new v2 store:
...
A v2 snapshot is portable between the JSON and SQL backends because it carries the shared logical model.
