...
货币组合只在启动时读取一次。管理员 reload 操作会更新配置快照，但不会重建运行中的登录表、指令、Vault 桥接或 placeholder 扩展；新增、删除或修改货币要重启后才会生效。
...
管理员 reload 操作会重新加载配置与语言快照，但不会在后端之间搬移数据，也不会重新打开存储后端。在服务创建时读取的设置——存储方式、货币、`settings.main-command-alias`、`leaderboard.enabled`——会保持启动时的值直到下次重启；reload 不会重建货币登录表、重新注册指令，也不会重新连接 Vault 与 placeholder 集成。
